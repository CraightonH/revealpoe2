...
Companion (§5): also surface each unique's **origin** (Kalguuran/Ezomyte/Vaal) from ggpk
`UniqueOrigins` as a `props.origin`, via the same overlay+bootstrap machinery — the bootstrap
already reads `UniqueOrigins` for the Vaal candidate filter.
...
Generates/refreshes **both** overlays from ggpk (+ poe2db for cultivated mods). Not invoked
by `dev`, `build:*`, `test`, or CI. Reads the `ggpk-poe2` mirror (`UniqueOrigins`, `Origin`).

- **`unique-origins.json`** — emitted directly from ggpk `UniqueOrigins` (all 126
  origin-bearing uniques), joined to RePoE by name → `{unique, vid, origin}`. No network.
- **`cultivated-uniques.json`** — the Vaal subset (below) enriched from poe2db.
...
### 5. Companion: unique **origin** prop

`UniqueOrigins` assigns each cultural-origin unique one of three origins (from the `Origin`
table): **Kalguuran, Ezomyte, Vaal**. This is worth surfacing on every unique that has one.

- **Coverage: 126 of 449 uniques** (Ezomyte 64, Vaal 48, Kalguuran 14). The other 323 are
  base-game / other-league uniques with *no* cultural origin in the data — `origin` is simply
  unset for them. Not a gap; "all uniques have an origin" is not attainable because the game
  only assigns these three origins to those 126.
- Origin is **ggpk-only** (RePoE has no origin field), so it uses the same committed-overlay
  pattern to keep ggpk out of the build.

**Overlay** `data/manual/unique-origins.json` (committed, `source: manual`):

```json
[ { "unique": "Atziri's Contempt", "vid": "FourUniqueSpear14_", "origin": "Vaal" } ]
```

- Generated by the bootstrap (below) as a **pure ggpk `UniqueOrigins` → JSON** transform —
  **no poe2db** needed for origins. Joined to RePoE uniques by name; keyed on `vid`.
- Builder (`manual.js`) attaches `node.props.origin` (`source: derived`,
  `via: manual:unique-origins`) where present. Same `vid` resolution + guards as §2;
  additionally, `origin` must be one of the three known values or the build fails.

**UI:** a subtle origin label/tag on the unique page header. Browse/filter-by-origin is a
...
| unique → origin (`unique-origins.json`) | `manual` (bootstrapped from ggpk `UniqueOrigins`) |
| `node.props.origin` | `derived` (`via: manual:unique-origins`) |
