...
- **Deploy/seed ordering for schema-affecting changes:** `deploy.yml` ships Worker code on *every* push to `main`, but D1 only picks up a new schema (table shapes, `meta` layout) on the next reseed, and reseeds are budgeted at once per `refresh-data.yml` run — never more than once a day (see the reseed row above). So a Worker change whose backend code depends on a new/changed table or `meta` shape must reseed D1 **before, or atomically with,** the push that ships it — trigger a manual reseed (the command above) as part of the same change, or hold the push until the next scheduled `refresh-data.yml` run has landed. Pushing the Worker first and reseeding later leaves the deployed Worker querying a schema D1 doesn't have yet, which fails in production while every local test (fs backend, and the d1 backend test's freshly-built `build/mcp.sql`) stays green.
