...
function init(root) {
  (root || document).querySelectorAll('.gem-quality-input').forEach(function (input) {
    if (input.dataset.qualityInputBound === 'true') return;
    input.dataset.qualityInputBound = 'true';
...
window.initGemQualityInput = init;
init(document);
