...
(function () {
  function init(root) {
    (root || document).querySelectorAll('.gem-level-select').forEach(function (sel) {
      if (sel.dataset.levelSelectBound === 'true') return;
      sel.dataset.levelSelectBound = 'true';
  ...
  window.initGemLevelSelect = init;
  init(document);
})();
