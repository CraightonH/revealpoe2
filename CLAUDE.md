...
Site action affordances attach to the card itself as a control strip directly below the header banner, so they are available everywhere the card renders — including hover tooltips — and never sit on top of the game art. Condensed grid cards additionally get a hover-revealed cluster on their border-box corner.
...
- **A second, separate deployable exists: the MCP graph server** (`workers/mcp/`), a standalone Cloudflare Worker that exposes `build/graph.json` as queryable MCP tools at `mcp.revealpoe2.com`, deployed by its own `deploy.yml` job (`deploy-mcp`) on every push to main — code only, it never touches D1. The backing D1 database (`revealpoe2-graph`) is reseeded **only** by the `seed-mcp` job in `refresh-data.yml`, kept isolated from the site refresh so a D1 failure can't block a deploy; see `docs/mcp-graph-server.md` for the runbook. The tool layer (`workers/mcp/tools/`) is pure and unit-tested under `node:test` against an in-memory `fsBackend`, with a parallel `d1Backend` exercised by an equivalence suite — the split's one rule is that the tool/backend-interface code never imports `node:fs`, so the exact same logic runs in tests and in the Worker against D1.
